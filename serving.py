...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        # serialized_tf_example = tf.placeholder(tf.string, x_test)
        serialized_tf_example = input_x
        values, idx = tf.nn.top_k(all_predictions, len(all_predictions))
        # nparr_x_test = np.array(x_test)
        class_tensor = tf.constant(x_raw)

        table = tf.contrib.lookup.index_to_string_table_from_tensor(class_tensor)
        classes = table.lookup(tf.to_int64(idx))

        # classes = tf.placeholder(dtype=tf.string, shape=[], name=table)
        # classes = table
        # values = all_predictions
